...
public class CountryArrayManager {
    ...
    public void remove(int index) {
        ...
        countryDataArray[this.length] = null;
        this.length--;
    }
    ...
    public CountryData[] sortIncreasingByArea() {
        ...
        Arrays.sort(newArray, new Comparator<CountryData>() {
            ...
            public int compare(CountryData left, CountryData right) {
                ...
                return (left.getArea() - right.getArea() > 0) ? 1 : -1;
            }
        });
        return newArray;
    }
    ...
    public CountryData[] sortIncreasingByGdp() {
        ...
        Arrays.sort(newArray, new Comparator<CountryData>() {
            @Override
            public int compare(CountryData left, CountryData right) {
                if (left.getGdp() - right.getGdp() == 0) {
                    return 0;
                }
                return (left.getGdp() - right.getGdp() > 0) ? 1 : -1;
            }
        });
        return newArray;
    }
    ...
    public CountryData[] filterContinent(String continent) {
        int countContinentCountry = 0;
//        ArrayList<CountryData> arrContinent = new ArrayList<>();
        for (CountryData countryData: countryDataArray) {
            if (continent.equals(countryData.getContinent())) {
                countContinentCountry++;
            }
        }
        int idx = 0;
        CountryData[] arrContinent = new CountryData[countContinentCountry];
        for (CountryData countryData: countryDataArray) {
            if (continent.equals(countryData.getContinent())) {
                arrContinent[idx] = countryData;
                idx++;
            }
        }
        return arrContinent;
    ...
    public CountryData[] filterMostPopulousCountries(int howMany) {
        CountryData[] newArray = this.sortDecreasingByPopulation();
        CountryData[] arrMostPopulousCountries = new CountryData[howMany];
        System.arraycopy(newArray, 0, newArray, 0, howMany);
        return arrMostPopulousCountries;
    }

    // Lọc ra howMany nước có dân số ít nhất.
    public CountryData[] filterLeastPopulousCountries(int howMany) {
        CountryData[] newArray = this.sortIncreasingByPopulation();
        CountryData[] arrLeastPopulousCountries = new CountryData[howMany];
        System.arraycopy(newArray, 0, newArray, 0, howMany);
        return arrLeastPopulousCountries;
    }

    // Lọc ra howMany nước có diện tích lớn nhất.
    public CountryData[] filterLargestAreaCountries(int howMany) {
        CountryData[] newArray = this.sortDecreasingByArea();
        CountryData[] arrLargestAreaCountries = new CountryData[howMany];
        System.arraycopy(newArray, 0, newArray, 0, howMany);
        return arrLargestAreaCountries;
    }

    // Lọc ra howMany nước có diện tích nhỏ nhất.
    public CountryData[] filterSmallestAreaCountries(int howMany) {
        CountryData[] newArray = this.sortIncreasingByArea();
        CountryData[] arrSmallestAreaCountries = new CountryData[howMany];
        System.arraycopy(newArray, 0, newArray, 0, howMany);
        return arrSmallestAreaCountries;
    }

    // Lọc ra howMany nước có GDP cao nhất.
    public CountryData[] filterHighestGdpCountries(int howMany) {
        CountryData[] newArray = this.sortDecreasingByGdp();
        CountryData[] arrHighestGdpCountries = new CountryData[howMany];
        System.arraycopy(newArray, 0, newArray, 0, howMany);
        return arrHighestGdpCountries;
    }

    // Lọc ra howMany nước có GDP thấp nhất.
    public CountryData[] filterLowestGdpCountries(int howMany) {
        CountryData[] newArray = this.sortIncreasingByGdp();
        CountryData[] arrLowestGdpCountries = new CountryData[howMany];
        System.arraycopy(newArray, 0, newArray, 0, howMany);
        return arrLowestGdpCountries;
    }
